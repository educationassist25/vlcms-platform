...
interface ActionPlanItem {
  ...
  affects_pairs: string[]; n_pairs_resolved: number;
  total_impact: number; difficulty: string; confidence: string;
}
...
const DIFF_COLOR: Record<string,string> = { easy:"var(--green)", moderate:"var(--amber)", advanced:"var(--red)" };
const CONF_COLOR: Record<string,string> = { high:"var(--accent)", medium:"var(--blue)", low:"var(--text-muted)" };
const RISK_COLOR: Record<string,string> = { critical:"#ff2d55", high:"#ff6b6b", medium:"#ffb347", low:"#00d4a4", none:"#57d9a3" };

function Badge({ text, color }: { text: string; color: string }) {
  return <span style={{ fontSize:10, padding:"1px 7px", borderRadius:4, background: color+"22", color, fontWeight:600 }}>{text}</span>;
}
    ...
    <div style={{
      ...
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:8, marginBottom:5 }}>
        ...
        <div style={{ display:"flex", gap:4, flexShrink:0 }}>
          <Badge text={s.confidence+" confidence"} color={CONF_COLOR[s.confidence]} />
          <Badge text={s.difficulty} color={DIFF_COLOR[s.difficulty]} />
        </div>
      </div>
      ...
      <div style={{ padding:"6px 10px", background:"var(--bg-primary)", borderRadius:6, borderLeft:"3px solid var(--accent)" }}>
        <div style={{ fontSize:9, fontWeight:700, color:"var(--accent)", marginBottom:2, letterSpacing:"0.04em" }}>SPECIFIC ACTION:</div>
...
export default function CoElutionResolverPanel({
  metaboliteIds, columnId, mobilePhaseId, gradient, flowRate, temperature, ionMode, autoRun = false
}: Props) {
  ...
  const diagnose = async () => {
    if (metaboliteIds.length < 2 || !columnId || !mobilePhaseId) {
      setError("Select at least 2 metabolites, a column, and a mobile phase first.");
      return;
    }
    ...
    try {
      ...
      setResult(res);
      setExpandedPair(res.n_critical_pairs > 0 ? 0 : null);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Diagnosis failed");
    } finally { setLoading(false); }
  };

  // Auto-run when parameters change (debounced 2s)
  useEffect(() => {
    if (!autoRun || metaboliteIds.length < 2 || !columnId || !mobilePhaseId) return;
    const paramsKey = JSON.stringify({ metaboliteIds, columnId, mobilePhaseId, gradient, flowRate, temperature, ionMode });
    if (paramsKey === lastParams) return;
    setLastParams(paramsKey);
    if (debounceRef.current) clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(() => { diagnose(); }, 2000);
    return () => { if (debounceRef.current) clearTimeout(debounceRef.current); };
  }, [metaboliteIds, columnId, mobilePhaseId, gradient, flowRate, temperature, ionMode, autoRun]);

  return (
    <div>
      {/* Header with Diagnose button */}
      <div style={{
        background:"linear-gradient(135deg, #ff6b6b10, #ffb34710)",
        border:"1px solid #ff6b6b30", borderRadius:10, padding:16, marginBottom:14,
      }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            <div style={{ fontSize:14, fontWeight:700, color:"var(--text-primary)", marginBottom:4 }}>
              🎯 Co-Elution Diagnosis & Resolution Engine
            </div>
            <div style={{ fontSize:12, color:"var(--text-secondary)" }}>
              Root-cause analysis + ranked, specific fixes. This is the core value of the platform.
            </div>
          </div>
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            {autoRun && loading && (
              <span style={{ fontSize:11, color:"var(--amber)" }}>⏳ Auto-updating…</span>
            )}
            <button onClick={diagnose} disabled={loading || metaboliteIds.length < 2} style={{
              padding:"10px 20px", borderRadius:8, fontSize:13, fontWeight:700,
              cursor: loading ? "not-allowed" : "pointer",
              background: loading ? "var(--border)" : "linear-gradient(135deg, #ff6b6b, #ff2d55)",
      ...
      {result && (
        <>
          ...
          <div style={{
            ...
            <span style={{ fontSize:28 }}>{result.status==="resolved" ? "✅" : "⚠️"}</span>
            <div>
              <div style={{ fontSize:13, fontWeight:700, color: result.status==="resolved" ? "var(--green)" : "var(--red)" }}>
                {result.status==="resolved" ? "Method is well-optimized — no critical co-elutions" : `${result.n_critical_pairs} critical co-elution(s) detected`}
              </div>
              <div style={{ fontSize:12, color:"var(--text-secondary)" }}>{result.summary}</div>
            </div>
          </div>

          {result.status !== "resolved" && result.n_critical_pairs > 0 && (
            <>
              {/* ── GLOBAL ACTION PLAN ── the main deliverable */}
              <div style={{ background:"var(--bg-card)", border:"1px solid var(--accent)50", borderRadius:10, overflow:"hidden", marginBottom:14 }}>
                <div style={{ padding:"10px 16px", background:"var(--accent-dim)", borderBottom:"1px solid var(--accent)30" }}>
                  <span style={{ fontSize:12, fontWeight:700, color:"var(--accent)", textTransform:"uppercase", letterSpacing:"0.05em" }}>
                    📋 Prioritized Action Plan — Apply These Steps in Order
                  </span>
                </div>
                  ...
                  {result.action_plan.map((item, i) => (
                    <div key={i} style={{
                      ...
                      <div style={{
                        ...
                        color: i===0 ? "#0f1117" : "var(--text-secondary)",
                        border: i===0 ? "none" : "1px solid var(--border)",
                      }}>{item.priority}</div>
                      <div style={{ flex:1 }}>
                        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:3, flexWrap:"wrap" }}>
                          <span style={{ fontSize:13, fontWeight:700, color:"var(--text-primary)" }}>{item.title}</span>
                          <Badge text={`Fixes ${item.n_pairs_resolved} pair${item.n_pairs_resolved>1?"s":""}`} color="var(--accent)" />
                          <Badge text={item.difficulty} color={DIFF_COLOR[item.difficulty]} />
                          <Badge text={item.confidence+" confidence"} color={CONF_COLOR[item.confidence]} />
                        </div>
                        <p style={{ fontSize:11, color:"var(--text-secondary)", lineHeight:1.5, marginBottom:3 }}>{item.description}</p>
                        <div style={{ fontSize:10, color:"var(--text-muted)" }}>Affects: {item.affects_pairs.join(" · ")}</div>
                      </div>
                      <div style={{ textAlign:"right", flexShrink:0 }}>
                        <div style={{ fontSize:16, fontWeight:700, color:"var(--accent)", fontFamily:"monospace" }}>+{item.total_impact.toFixed(2)}</div>
                        <div style={{ fontSize:9, color:"var(--text-muted)" }}>Rs gain</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={{ background:"var(--bg-card)", border:"1px solid var(--border)", borderRadius:10, overflow:"hidden" }}>
                <div style={{ padding:"10px 14px", borderBottom:"1px solid var(--border)" }}>
                  <span style={{ fontSize:12, fontWeight:700, color:"var(--text-secondary)", textTransform:"uppercase", letterSpacing:"0.05em" }}>
                    🔬 Root-Cause Analysis — {result.diagnoses.length} Co-eluting Pairs
                  </span>
                </div>
                  ...
                  <div key={i} style={{ borderBottom: i < result.diagnoses.length-1 ? "1px solid var(--border)" : "none" }}>
                    <button onClick={() => setExpandedPair(expandedPair===i ? null : i)} style={{
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                        <span style={{ color: expandedPair===i ? "var(--accent)" : "var(--text-muted)", fontSize:12 }}>
                          {expandedPair===i ? "▼" : "▶"}
                        </span>
                    ...
                    {expandedPair===i && (
                      <div style={{ padding:"0 14px 16px 36px" }}>
                        ...
                        <div style={{ padding:"10px 12px", background:"var(--bg-secondary)", borderRadius:8, marginBottom:10, borderLeft:"3px solid var(--amber)" }}>
                          <div style={{ fontSize:10, fontWeight:700, color:"var(--amber)", marginBottom:4, letterSpacing:"0.04em" }}>WHY THEY CO-ELUTE</div>
                          <p style={{ fontSize:12, color:"var(--text-secondary)", lineHeight:1.6 }}>{d.mechanism_explanation}</p>
                        </div>

                        {/* Similarity factors */}
                        {d.similarity_factors.length > 0 && (
                          <div style={{ marginBottom:12 }}>
                            <div style={{ fontSize:10, fontWeight:700, color:"var(--text-muted)", marginBottom:6, letterSpacing:"0.04em" }}>PHYSICOCHEMICAL SIMILARITIES</div>
